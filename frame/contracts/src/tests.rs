	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			pretty_assertions::assert_eq!(System::events(), vec![
				...
				EventRecord {
					phase: Phase::Initialization,
					event: MetaEvent::balances(pallet_balances::RawEvent::Endowed(addr, 1_000_000)),
					topics: vec![],
				},
				EventRecord {
					phase: Phase::Initialization,
...
fn restoration(test_different_storage: bool, test_restore_to_with_dirty_storage: bool) {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let addr_bob = Contracts::contract_address(&ALICE, &set_rent_code_hash, 0);
			...
			let addr_django = Contracts::contract_address(&CHARLIE, &restoration_code_hash, 0);
...
fn deploy_and_call_other_contract() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let addr = Contracts::contract_address(&ALICE, &caller_code_hash, 0);
...
fn destroy_contract_and_transfer_funds() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let addr_bob = Contracts::contract_address(&ALICE, &caller_code_hash, 0);
			let addr_charlie = Contracts::contract_address(&addr_bob, &callee_code_hash, 0);
...
fn call_return_code() {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		...
		let addr_bob = Contracts::contract_address(&ALICE, &caller_hash, 0);
...
fn instantiate_return_code() {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		...
		let addr = Contracts::contract_address(&ALICE, &caller_hash, 0);
